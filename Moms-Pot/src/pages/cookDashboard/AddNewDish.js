        ...
        <div className="mx-32 border-b-2 border-white mt-4">
          ...
          <div className="flex space-x-2 mt-6 mb-2">
            <div className="flex justify-start mx-10">
              <div className="text-xl font-semibold">Add new dish</div>
            </div>
          </div>
        </div>
          ...
          <div className=" w-1/2 flex justify-start">
            ...
            <div>
              ...
              onClick={()=>{
                ...
                axios.post("http://localhost:5000/dishes/createDish",dishInfo).then((response)=>{
                  console.log(response)
                  if(dishLink.current.files[0]){
                    const fd = new FormData()
                    fd.append('image', dishLink.current.files[0],response.data._id + ".jpg")
                    axios.post("http://localhost:5000/dishImg",fd).then((response)=>{                                            
